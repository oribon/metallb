...
def build(ctx, binaries, architectures, registry="quay.io", repo="metallb", tag="dev"):
    ...
    for arch in architectures:
        ...
        if "speaker" in binaries:
            run(
                "cp frr-reloader/frr-reloader.sh build/{arch}/speaker/".format(
                    arch=arch,
                )
            )
...
def dev_env(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manifest_filename = "metallb_frr.yaml" if bgp_type == "frr" else "metallb.yaml"
...
def e2etest(
    ...
    testrun = run(
        ...
        "go test {} {} --provider=local --kubeconfig={} --service-pod-port={} -ipv4-service-range={} -ipv6-service-range={} {}".format(
            ginkgo_focus,
            ginkgo_skip,
            kubeconfig,
            service_pod_port,
