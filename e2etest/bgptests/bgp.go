...
var _ = ginkgo.Describe("BGP", func() {
	...
	ginkgo.DescribeTable("ServiceBGPStatus", func(ipFamily ipfamily.Family, poolAddresses []string, tweak testservice.Tweak) {
		validateStatusesFor := func(nodes, peers []string, svc *corev1.Service, expectNoResources bool) error {
			for _, n := range nodes {
				...
				if !cmp.Equal(peers, s.Status.Peers) {
					return fmt.Errorf("expected status peers to be %v, got %v for node %s\n diff: %s", peers, s.Status.Peers, n, cmp.Diff(peers, s.Status.Peers))
				}
			}
		...
		Expect(err).ToNot(HaveOccurred())
		Eventually(func() error {
			return validateStatusesFor(nodesNames, peersNames[1:], svc, len(peers) == 1)
		}, 2*time.Minute, 2*time.Second).ShouldNot(HaveOccurred())
		...
		Eventually(func() error {
			return validateStatusesFor([]string{nodesNames[0]}, []string{}, svc, true)
		}, 2*time.Minute, 2*time.Second).ShouldNot(HaveOccurred())
		Eventually(func() error {
			return validateStatusesFor(nodesNames[1:], peersNames[1:], svc, len(peers) == 1)
		}, 2*time.Minute, 2*time.Second).ShouldNot(HaveOccurred())
		...
		svcDeleted = true
		Eventually(func() error {
			return validateStatusesFor(nodesNames, []string{}, svc, true)
		}, 2*time.Minute, 2*time.Second).ShouldNot(HaveOccurred())
	},
		ginkgo.Entry("IPV4", ipfamily.IPv4, []string{v4PoolAddresses}, func(_ *corev1.Service) {}),
		ginkgo.Entry("IPV6", ipfamily.IPv6, []string{v6PoolAddresses}, func(_ *corev1.Service) {}),
