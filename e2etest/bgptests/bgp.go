...
var _ = ginkgo.Describe("BGP", func() {
	...
	ginkgo.Context("MetalLB FRR rejects", func() {
		...
		table.DescribeTable("any routes advertised by any neighbor", func(addressesRange, toInject string, pairingIPFamily ipfamily.Family) {
			resources := metallbconfig.ClusterResources{
				Pools: []metallbv1beta1.IPAddressPool{
					{
						ObjectMeta: metav1.ObjectMeta{
			...
			table.Entry("IPV4", "192.168.10.0/24", "172.16.1.1/32", ipfamily.IPv4),
	...
	ginkgo.Context("MetalLB allows adding extra FRR configuration", func() {
		...
		table.DescribeTable("to accept any routes advertised by any neighbor", func(addressesRange, toInject string, pairingIPFamily ipfamily.Family, when whenApply) {
			resources := metallbconfig.ClusterResources{
				Pools: []metallbv1beta1.IPAddressPool{
					{
						ObjectMeta: metav1.ObjectMeta{
			...
			checkRoutesAreInjected := func() error {
				for i, c := range FRRContainers {
					injected, _ := isRouteInjected(speakerPods, pairingIPFamily, fmt.Sprintf(toInject, i+1), "")
					if !injected {
						return fmt.Errorf("route not injected from %s", c.Name)
					}
					injected, podName := isRouteInjected(speakerPods, pairingIPFamily, toFilter, "")
					if injected {
			...
			table.Entry("IPV4 - before config", "192.168.10.0/24", "172.16.1.%d/32", ipfamily.IPv4, before),
			table.Entry("IPV6 - before config", "fc00:f853:0ccd:e799::/116", "fc00:f853:ccd:e800::%d/128", ipfamily.IPv6, before),
			table.Entry("IPV4 - after config", "192.168.10.0/24", "172.16.1.%d/32", ipfamily.IPv4, after),
			table.Entry("IPV6 - after config", "fc00:f853:0ccd:e799::/116", "fc00:f853:ccd:e800::%d/128", ipfamily.IPv6, after),
		)
	})
